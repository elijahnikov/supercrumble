...
export class ReviewCommentResolver {
    ...
    async voteStatus(
        ...
        const reviewCommentUpvote = await reviewCommentUpvoteLoader.load({reviewCommentId: reviewComment.id, userId: req.session.userId})
    ...
    async createReviewComment(
        ...
        //     await tm.query(
        //         `
        //             insert into post_comment ("parentId", "creatorId", "postId", "text")
        //             values ($1, $2, $3, $4);
        //         `, [input.parentId, req.session.userId, input.postId, input.text]
        //     )
    ...
    async reviewComments(
        ...
        @Arg('order', () => String, {nullable: true, defaultValue: 'DESC'}) order: 'ASC' | 'DESC' | undefined
    ...
    async deleteReviewComment(
        ...
        const reviewComment = await ReviewComment.findOne({where: {id: id}})
    ...
    async reviewCommentVote(
        ...
        const reviewCommentUpvote = await ReviewCommentUpvote.findOne({where: {reviewCommentId, userId}})
    ...
    reviewComment(@Arg('id', () => Int) id: number): Promise<ReviewComment | undefined> { 
