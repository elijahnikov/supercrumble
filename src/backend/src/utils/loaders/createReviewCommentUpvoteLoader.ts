...
export const createReviewCommentUpvoteLoader = () =>
    new DataLoader<
        { reviewCommentId: number; userId: number },
        ReviewCommentUpvote | null
    >(async (keys) => {
        ...
        upvotes.forEach((upvote) => {
            upvoteIdsToUpvote[`${upvote.userId}|${upvote.reviewCommentId}`] =
                upvote;
        });
        return keys.map(
            (key) => upvoteIdsToUpvote[`${key.userId}|${key.reviewCommentId}`]
        );
    });
